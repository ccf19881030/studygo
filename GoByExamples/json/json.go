...
func main() {
	...
	// 码的输出只包含可导出的字段，并且默认使用字段名作为 JSON 数据的键名。
	res1D := &response1{
		Page: 1,
	...
	// 现在来看看将JSON数据解码为Go值的过程。
	// 这是一个普通数据结构的解码例子。
	byt := []byte(`{"num":6.13,"strs":["a","b"]}`)

	// 我们需要提供一个JSON包可以存放解码数据的变量。
	// 这里的map[string]interface{}是一个键为string,值为任意值的map。
	...
	// 为了使用解码map的值，我们需要将他们进行适当的类型转换。例如，这里我们将num的值转换成float64类型。
	num := dat["num"].(float64)
	fmt.Println(num)
